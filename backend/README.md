...
### Постоянное хранилище загрузок (S3)

По умолчанию файлы (фото офферов, логотипы, фото отзывов) сохраняются на диск. На PaaS (Render и т.п.) диск эфемерный — после перезапуска файлы пропадают. Чтобы загрузки сохранялись, настройте S3-совместимое хранилище (AWS S3, Yandex Object Storage, Cloudflare R2 и т.д.).
...
# Для AWS S3 переменные S3_ENDPOINT не нужны.
```

После этого новые загрузки будут уходить в бакет и отдаваться оттуда же; старые файлы с диска по-прежнему отдаются с сервера, если их ещё не перенесли в S3.
